(function() {
    ...
    if (window.__weighingsPageInitialized) {
        return;
    }
    window.__weighingsPageInitialized = true;

    const config = window.weighingsConfig || {};
    ...
    function loadPools() {
        $.ajax({
            url: apiUrl('api/weighings.php?action=get_pools'),
            method: 'GET',
            dataType: 'json',
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                                <h5 class="mb-0">
                                    Навески для бассейна "${escapeHtml(pool.name || pool.pool_name || `Бассейн ${pool.id}`)}"
    ...
    function loadRecords(poolId) {
        ...
        $.ajax({
            url: apiUrl(`api/weighings.php?action=list&pool_id=${poolId}`),
            method: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    ...
                    tbody.html(`<tr><td colspan="${colSpan}" class="text-center текст-muted">Не удалось загрузить данные</td></tr>`);
                }
            },
            error: function(xhr, status, error) {
    ...
    function openEditModal(id) {
        ...
        $.ajax({
            url: apiUrl(`api/weighings.php?action=get&id=${id}`),
            method: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
    ...
    function saveRecord() {
        ...
        $.ajax({
            url: apiUrl(`api/weighings.php?action=${action}`),
            method: 'POST',
    ...
    function deleteRecord(id) {
        ...
        $.ajax({
            url: apiUrl('api/weighings.php?action=delete'),
            method: 'POST',
            contentType: 'application/json',
